...
public class DeathMenuControllerTest {
    private DeathMenuController controller;
    private DeathMenu model;
    ...
    @BeforeEach
    public void setUp() {
        this.model = Mockito.mock(DeathMenu.class);
        this.controller = new DeathMenuController(model);
    ...
    void tickExecTest(@ForAll @IntRange(min = 0, max = 1) int selectedOption) throws Exception {
        try (MockedStatic<GUI> mockedGUI = Mockito.mockStatic(GUI.class);
            ...
            DeathMenu mockedModel = Mockito.mock(DeathMenu.class);
            DeathMenuController controller1 = new DeathMenuController(mockedModel);
            ...
            for (DeathMenu.DeathMenuOptions option : DeathMenu.DeathMenuOptions.values())
            ...
            switch (DeathMenu.DeathMenuOptions.valueOf(options.get(selectedOption))) {
                case Retry -> {
                    // back to arena
