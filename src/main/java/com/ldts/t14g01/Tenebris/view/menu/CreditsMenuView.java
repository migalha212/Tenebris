...
public class CreditsMenuView extends View<Menu> {
    ...
    protected void drawElements() {
        ...
        for (int i = 0; i < titleLines.size(); i++)
            gui.drawText(
                    ucInfoLines.get(i),
                    new Vector2D(gui.getWindowSize().x() - offsetX - ucInfoLines.get(i).length(), centerY - 8 + i),
                    GUI.Colors.WHITE,
                    GUI.Colors.BLACK
            );
        ...
        for (int i = 0; i < professorsLines.size(); i++)
            gui.drawText(
                    ...
                    new Vector2D(gui.getWindowSize().x() - offsetX - professorsLines.get(i).length(), centerY - 1 + i),
                    GUI.Colors.WHITE,
                    GUI.Colors.BLACK
            );
        ...
        gui.drawText(
                "Back",
                new Vector2D(centerX - backText.length() / 2, gui.getWindowSize().y() - 3),
                GUI.Colors.YELLOW,
                GUI.Colors.BLACK
        );
    }
